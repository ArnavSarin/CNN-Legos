...
#        return df
    ...
    #CONVOLUTION PARALLELISM FOR FEATURE LEARNING
...
#        conv_img[(i * (ccol * crow)) + (j * ccol) + k] += fake_img[((i/fmatrix) * (icol * irow)) + (row * icol + col)+(j*icol+k)] * fake_filters[((i%fmatrix) * (fcol * frow)) + (row * fcol) + col];
        ...
        prg.multiplymatrices(self.queue, cmatrix.shape ,None, np.int32(cmatrix_size/fmatrix_size), cmatrix_size, imatrix_size, irow, icol, frow, fcol, crow, ccol,cl_a, cl_b, cl_c)
    ...
    #CONVOLUTION PARALLELISM FOR BACKPROPOGATION IN RELATION TO FILTER CHANGE
    ...
    #MAX POOLING PARALLELISM FOR FEATURE LEARNING
    def parallel_pooling(self, pre_pool, aft_pool, pool_x, pool_y):
        ...
        #RUN PARALLELISM ALGORITHM IN C
        prg3.pooling_parallelism(self.queue, aft_pool.shape ,None, np.int32(self.pool_height), np.int32(self.pool_length), prep_row, prep_col, aftp_row, aftp_col, cl_a, cl_b, cl_c, cl_d)
    
        #CREATE POOLING ARRAY AND COORDINATE ARRAY FILLED WITH ZEROS
        aft_pool = np.zeros_like(aft_pool,dtype=np.float32)
        aft_pool = np.empty_like(aft_pool)
        
        pool_x = np.zeros_like(pool_x,dtype=np.float32)
        pool_x = np.empty_like(pool_x)
        
        pool_y = np.zeros_like(pool_y,dtype=np.float32)
    ...
    #CONVOLUTION PARALLELISM FOR BACKPROPOGATION IN RELATION TO LOSS CHANGE BASED ON INPUT
    ...
    #DERIVATIVE MAX POOLING PARALLELISM FOR BACKPROPOGATION
    def max_pool_deriv_parallel(self, aft_p_loss, pre_p_loss, pool_x, pool_y):
        
        #CONVERT MATRIX TO FLOAT32
        aft_p_loss = aft_p_loss.astype(np.float32)
        pre_p_loss = pre_p_loss.astype(np.float32)
        
        #CONVERT MATRIX TO INT32
        pool_x = pool_x.astype(np.int32)
        pool_y = pool_y.astype(np.int32)
        
        #CREATE BUFFER FOR EACH ARRAY
        mf = cl.mem_flags
        cl_a = cl.Buffer(self.ctx, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf = pool_x.flatten())
        cl_b = cl.Buffer(self.ctx, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf = pool_y.flatten())
        cl_c = cl.Buffer(self.ctx, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf = aft_p_loss.flatten())
        cl_d = cl.Buffer(self.ctx, mf.WRITE_ONLY, pre_p_loss.flatten().nbytes)
        
        prg5 = cl.Program(self.ctx, """
        ...
        #CONVERTING AFTER MAX POOLING LOSS DIMENSIONS TO INT32
        if(aft_p_loss.ndim == 3):
            aftp_size = np.int32(aft_p_loss.shape[0])
            aftprow = np.int32(aft_p_loss.shape[1])
            aftpcol = np.int32(aft_p_loss.shape[2])
        else:
            aftp_size = np.int32(1)
            aftprow = np.int32(aft_p_loss.shape[0])
            aftpcol = np.int32(aft_p_loss.shape[1])
               
               
        #CONVERTING PRE MAX POOLING LOSS DIMENSIONS DIMENSIONS TO INT32
        if(pre_p_loss.ndim == 3):
            prep_size = np.int32(pre_p_loss.shape[0])
            preprow = np.int32(pre_p_loss.shape[1])
            prepcol = np.int32(pre_p_loss.shape[2])
        else:
            prep_size = np.int32(1)
            preprow = np.int32(pre_p_loss.shape[0])
            prepcol = np.int32(pre_p_loss.shape[1])
               
               
        #RUN PARALLELISM ALGORITHM IN C
        prg5.max_pooling_deriv_parallel(self.queue, pre_p_loss.shape ,None, np.int32(self.pool_height), np.int32(self.pool_length), aftprow, aftpcol, preprow, prepcol, cl_a, cl_b, cl_c, cl_d)
    
        #CREATE ARRAY OF PRE MAX POOLING LOSS FILLED WITH ZEROS
        pre_p_loss = np.zeros_like(pre_p_loss,dtype=np.float32)
        pre_p_loss = np.empty_like(pre_p_loss)
        
        #COPY OVER PARALLELISM ALGORITHM RESULT TO CONV IMAGE ARRAY
        cl.enqueue_copy(self.queue, pre_p_loss , cl_d)
    ...
    def __init__(self):
        ...
        self.num_filtersL1 = 1
        self.num_filtersL2 = 1
        
        #LAYER 1 KERNEL INITIALIZATION
#        self.filterL1 = np.random.rand(self.num_filtersL1, self.kernel_height, self.kernel_length)
        self.filterL1 = np.random.uniform(-1,1,(self.num_filtersL1, self.kernel_height, self.kernel_length))
        
        print(self.filterL1)
        
        #TESTING NEW INITIALIZATION CODE
        self.filterL1 = self.filterL1 * (math.sqrt(1/(self.kernel_height*self.kernel_length)))
        
        print("\nFILTER L1 AFTER XAVIER")
        print(self.filterL1)
        
        #LAYER 2 KERNEL INITIALIZATION
#        self.filterL2 = np.random.rand(self.num_filtersL2, self.kernel_height, self.kernel_length)
        self.filterL2 = np.random.uniform(-1,1,(self.num_filtersL2, self.kernel_height, self.kernel_length))
        
        #TESTING NEW INITIALIZATION CODE
        self.filterL2 = self.filterL2 * (math.sqrt(1/(self.kernel_height*self.kernel_length)))
...
#        length = (((self.input_length - self.kernel_length + 1)/self.pool_length) - self.kernel_length + 1)/ self.pool_length
        height = (((self.input_height)/ self.pool_height)/self.pool_height)
        length = (((self.input_length)/ self.pool_length)/self.pool_length)
    ...
    #LAYER 1 OF FEATURE LEARNING
        ...
        self.input_img = input_img / 255
        
        #PAD INPUT IMG TO KEEP DIMENSIONS
        self.input_img = np.pad(self.input_img,((1,1),(1,1)),constant_values=0)
        
        print(self.input_img[40][40])
...
#        self.C1_length = self.input_length - self.kernel_length + 1
        self.C1_height = self.input_height
        self.C1_length = self.input_length
        self.C1 = np.zeros((self.num_filtersL1, self.C1_height, self.C1_length))

        
        #CONVOLUTION FUNCTION PARALLELISM
        self.C1 = self.conv_parallelism(self.filterL1,self.input_img,self.C1)
        

        #ReLU ACTIVATION FUNCTION
        self.activ_C1 = self.ReLU(self.C1)
#        self.activ_C1 = self.sigmoid(self.C1)
    ...
    #LAYER 2 OF FEATURE LEARNING
        ...
        self.P1 = np.pad(self.P1,((0,0),(1,1),(1,1)),constant_values=0)
...
#        self.C2_length = int(self.P1_length - self.kernel_length + 1)
        self.C2_height = int(self.P1_height)
        self.C2_length = int(self.P1_length)
...
#        self.activ_C2 = self.sigmoid(self.C2)
...
#        dpooling2 = self.dmax_pooling(self.activ_C2,self.P2,bp_error)
        dpooling2 = self.max_pool_deriv_parallel(bp_error, self.activ_C2, self.p2_x, self.p2_y)
        
        #DERIVATIVE OF RELU
        dRelU_2 = self.d_ReLU(self.activ_C2)
#        dRelU_2 = self.d_sigmoid(self.activ_C2)
...
#        dpooling = self.dmax_pooling(self.activ_C1,self.P1,C2_error)
        dpooling = self.max_pool_deriv_parallel(C2_error, self.activ_C1, self.p1_x, self.p1_y)
        
        #DERIVATIVE OF RELU
        dRelU = self.d_ReLU(self.activ_C1)
#        dRelU = self.d_sigmoid(self.activ_C1)
        ...
        if(inp.strip().upper()== "T"):
            ...
            for i in range (0,500):
                for key in LEGO_BLOCKS:
                    cv_img = cv2.imread("/Users/arnavsarin/Desktop/CNN/80x80_ROTATED_TRAINING/" + key +  str(LEGO_BLOCKS.get(key)).zfill(4) + ".png",0)
            ...
            for i in range (0,350):
                for key in LEGO_BLOCKS_TEST_ALL:
                    cv_img2 = cv2.imread("/Users/arnavsarin/Desktop/CNN/80x80_350/" + key + str(LEGO_BLOCKS_TEST_ALL.get(key)).zfill(4) + ".png",0)
            ...
            for i in range (0,50):
                for key in LEGO_BLOCKS_TEST_ALL:
                    cv_img3 = cv2.imread("/Users/arnavsarin/Desktop/CNN/80x80_50/" + key + str(LEGO_BLOCKS_TEST_ALL.get(key)).zfill(4) + ".png",0)
...
#    main()
    ...
    CNN = Convolutional_Neural_Network()
    cv_img = cv2.imread("/Users/arnavsarin/Desktop/CNN/80x80_ROTATED_TRAINING/2x2-0001.png",0)
    actual = CNN.Feature_Learning(cv_img)

    actual_conv = np.zeros((CNN.num_filtersL1, CNN.C1_height, CNN.C1_length))

    for f in range (CNN.num_filtersL1):
        for l in range (CNN.C1_length):
            for h in range (CNN.C1_height):
                filtered_subset = CNN.input_img[h:h+CNN.kernel_height,l:l+CNN.kernel_length] * CNN.filterL1[f]
                actual_conv[f,h,l] = np.sum(filtered_subset)

    print("\nCHECK FIRST CONVOLUTION")
    print(np.array_equal(np.around(CNN.C1.astype(np.float16),decimals=3), np.around(actual_conv.astype(np.float16),decimals=3), equal_nan=True))
    print(CNN.C1.shape == actual_conv.shape)
    #    for a in range (0,CNN.num_filtersL1):
    #        for b in range (0,CNN.C1_length):
    #            for c in range (0,CNN.C1_height):
    #                if(np.round(CNN.C1[a,b,c],decimals=3) != np.round(actual_conv[a,b,c],decimals=3)):
    #                    print(CNN.C1[a,b,c])
    #                    print(actual_conv[a,b,c])

    actual_conv = CNN.ReLU(actual_conv)

    actual_pool = np.zeros((CNN.num_filtersL1, CNN.P1_height, CNN.P1_length))

    for f in range (CNN.num_filtersL1):
        for l in range (CNN.P1_length):
            for h in range (CNN.P1_height):
                h_iter = h * CNN.pool_height
                l_iter = l * CNN.pool_length
                actual_pool[f,h,l] = CNN.max_pooling(actual_conv[f, h_iter:h_iter+CNN.pool_height,l_iter:l_iter+CNN.pool_length])

    print("\nCHECK FIRST MAX POOLING")
    actual_pool = np.pad(actual_pool,((0,0),(1,1),(1,1)),constant_values=0)
    print(np.array_equal(np.around(CNN.P1.astype(np.float16),decimals=3), np.around(actual_pool.astype(np.float16),decimals=3), equal_nan=True))

    print(CNN.P1.shape == actual_pool.shape)

    actual_conv2 = np.zeros((CNN.num_filtersL1 * CNN.num_filtersL2, CNN.C2_height, CNN.C2_length))

    for f in range (CNN.num_filtersL1 * CNN.num_filtersL2):
        for l in range (CNN.C2_length):
            for h in range (CNN.C2_height):
                filtered_subset = actual_pool[int(f%CNN.num_filtersL1),h:h+CNN.kernel_height,l:l+CNN.kernel_length] * CNN.filterL2[int(f/CNN.num_filtersL1)]
                actual_conv2[f,h,l] = np.sum(filtered_subset)


    print("\nCHECK SECOND CONVOLUTION")
    print(np.array_equal(np.around(CNN.C2.astype(np.float16),decimals=3), np.around(actual_conv2.astype(np.float16),decimals=3), equal_nan=True))
    print(CNN.C2.shape == actual_conv2.shape)

    actual_conv2 = CNN.ReLU(actual_conv2)

    actual_pool2 = np.zeros((CNN.num_filtersL1 * CNN.num_filtersL2, CNN.P2_height, CNN.P2_length))

    for f in range (CNN.num_filtersL1 * CNN.num_filtersL2):
        for l in range (CNN.P2_length):
            for h in range (CNN.P2_height):
                h_iter = h * CNN.pool_height
                l_iter = l * CNN.pool_length
                actual_pool2[f,h,l] = CNN.max_pooling(actual_conv2[f, h_iter:h_iter+CNN.pool_height,l_iter:l_iter+CNN.pool_length])

    print("\nCHECK SECOND MAX POOLING")
    print(np.array_equal(np.around(CNN.P2.astype(np.float16),decimals=3), np.around(actual_pool2.astype(np.float16),decimals=3), equal_nan=True))
    print(CNN.P2.shape == actual_pool2.shape)

#    FORWARD PASS BEFORE FULLY CONNECTED LAYER SEEMS TO WORK


    CNN.Classification()

    factual_pool2 = actual_pool2.flatten()

    h1_nobias = np.dot(factual_pool2,CNN.w1)
    h1_bias = h1_nobias + CNN.b1
    h1 = CNN.sigmoid(h1_bias)

    print("\nCHECK HIDDEN LAYER 1")
    print(np.array_equal(np.around(CNN.h1.astype(np.float16),decimals=3), np.around(h1.astype(np.float16),decimals=3), equal_nan=True))

    #CALCULATE OUTPUT LAYER
    z1_nobias = np.dot(h1,CNN.w2)
    z1_bias = z1_nobias + CNN.b2
    o1 = CNN.softmax(z1_bias)

    print("\nCHECK OUTPUT LAYER")
    print(np.array_equal(np.around(CNN.o1.astype(np.float16),decimals=3), np.around(o1.astype(np.float16),decimals=3), equal_nan=True))
    
#    CLASSIFICATION AND FULLY CONNECTED LAYER FORWARD PASS SEEM TO WORK
    
    
    expect = CNN.label("2x2-")
    
    #CROSS ENTROPY LOSS
    error = -1 * (expect/o1)
    
    #SOFTMAX DERIVATION
    dsoftmax = CNN.d_softmax(z1_bias)
    
    #WEIGHT CHANGE CALCULATION (JACOBIAN MULTIPLICATION)
    m_jacobian = (error * dsoftmax)
    sum_jacobian = np.sum(m_jacobian, axis=1)
    c_w2 = np.dot(h1.reshape(CNN.hiddenL1_neurons,1),sum_jacobian.reshape(1,CNN.o_neurons))
    
    #BIAS CHANGE CALCULATION
    c_b2 = sum_jacobian

    #SIGMOID DERIVATION
    dsigmoid = CNN.d_sigmoid(h1)
    
    #ERROR CALCULATION
    fin_error = sum_jacobian * CNN.w2
    fin_error = np.sum(fin_error, axis=1)
    
    #WEIGHT CHANGE CALCULATION
    #(DERIV OF COST/ DERIV OF ACTIVATION OUTPUT) * (DERIV OF ACTIVATION OUTPUT / DERIV OF ACTIVATION INPUT)
    dcost_actinput = fin_error * dsigmoid
    c_w1 = np.dot(factual_pool2.reshape(int(CNN.finput_neurons),1),dcost_actinput.reshape(1,CNN.hiddenL1_neurons))
    
    #BIAS CHANGE CALCULATION
    c_b1 = dcost_actinput

    CNN.Backpropogation(expect,.001)

    print("\nCHECK change in W2")
    print(np.array_equal(np.around(CNN.c_w2.astype(np.float16),decimals=3), np.around(c_w2.astype(np.float16),decimals=3), equal_nan=True))

    print("\nCHECK change in W1")
    print(np.array_equal(np.around(CNN.c_w1.astype(np.float16),decimals=3), np.around(c_w1.astype(np.float16),decimals=3), equal_nan=True))

    print("\nCHECK change in B2")
    print(np.array_equal(np.around(CNN.c_b2.astype(np.float16),decimals=3), np.around(c_b2.astype(np.float16),decimals=3), equal_nan=True))
    
    print("\nCHECK change in B1")
    print(np.array_equal(np.around(CNN.c_b1.astype(np.float16),decimals=3), np.around(c_b1.astype(np.float16),decimals=3), equal_nan=True))

    bp_error = fin_error * CNN.w1
    bp_error = np.sum(bp_error,axis = 1)
    bp_error = bp_error.reshape(CNN.num_filtersL1 * CNN.num_filtersL2, CNN.P2_height, CNN.P2_length)
